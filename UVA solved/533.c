...
char *receive(char *to, char *from){
    char temp=0, ch, *ptr=to;

    for( ; *from && *from!='='; from++, to++) *to=*from;
    *to=0;
    return from;
}

void eval_equ(double xval, double *ans, char *left, char *right){
    double x1, x2;
...
int main(){
    double x1, x2;
    char exp1[SIZE+1], exp2[SIZE+1], array[SIZE*2];
    int test=0;

    while(gets(array)){
        if(test) puts("");
        printf("Equation #%d\n", ++test);
        receive(exp2, receive(exp1, array)+1);

        eval_equ(0, &x1, exp1, exp2);
        eval_equ(1, &x2, exp1, exp2);

        if(fabs(x1-x2)<1e-7){
            if(fabs(x1)<1e-7) puts("Infinitely many solutions.");
