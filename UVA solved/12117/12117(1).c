...
long long calcWays( int curRow, int rowconf, int nextconf, int thenconf ){
    ...
    if ( rowconf == 0 && nextconf == 0 && thenconf == 0 && cached[curRow] )
        return cached[curRow];
    if ( curRow == 0 && rowconf == 0 && nextconf == 0 && thenconf == 0 ) return 1;
    if ( curRow <= 0 ) return 0;
    if ( visited[curRow][rowconf][nextconf][thenconf] )
